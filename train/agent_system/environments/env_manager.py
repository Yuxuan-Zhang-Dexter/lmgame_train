...
class SokobanEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self):
        ...
        if self.is_multi_modal:
            ...
            self.pre_text_obs = self.envs.render(mode='rgb_array')
            observations = {
...
def make_envs(config):
    ...
    if "gym_cards" in config.env.env_name.lower():
        ...
        import json
        sokoban_config_path = "agent_system/environments/env_package/sokoban/custom_02_sokoban/game_env_config_train.json"
        env_kwargs = json.loads(open(sokoban_config_path).read())['env_kwargs']
        _envs = build_sokoban_envs(config.env.seed, config.data.train_batch_size, group_n, mode='rgb_array', is_train=True, env_kwargs=env_kwargs)
        _val_envs = build_sokoban_envs(config.env.seed + 1000, config.data.val_batch_size, 1, mode='rgb_array', is_train=False, env_kwargs=env_kwargs)
